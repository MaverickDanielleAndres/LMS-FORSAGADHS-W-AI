...
if (isset($_POST['subbed'])) {
    ...
    // Rename the file with subject + unit + timestamp to allow same unit number
    $materialcode = $subcode . "_" . $unitno . "_" . time();
    $materialfile = $materialcode . "_MATERIAL.pdf";

    try {
        $sql = "INSERT INTO `studymaterialmaster`
                (`SubjectCode`, `SubjectUnitNo`, `MaterialCode`, `SubjectUnitName`, `MaterialFile`, `MaterialUploadDate`)
                VALUES ('$subcode', '$unitno', '$materialcode', '$unitname', '$materialfile', '$dt')";

        $run = mysqli_query($conn, $sql);

        if ($run) {
            if ($fs_error === 0 && $fs_size <= 5000000) {
                move_uploaded_file($fs_name, "../src/uploads/studymaterial/" . $materialfile);
                ...
                echo "<script>
                        showCustomAlert(
                            'File upload failed. Check file size or error.',
                            'warning',
                            'Upload Warning'
                        );
                      </script>";
            ...
            echo "<script>
                    showCustomAlert(
                        'Error occurred, study material was not added.',
                        'error',
                        'Database Error',
                        function () {
                            window.location.href = 'add_material.php?subcode=$subcode';
                        }
                    );
                  </script>";
        }
    } catch (Exception $e) {
        // UNEXPECTED EXCEPTION MODAL
        echo "<script>
                showCustomAlert(
