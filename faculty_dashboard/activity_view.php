				...
				<div class="row justify-content-center">
					<div class="col-12	">
						...
						if (isset($ttid)) {
							...
							$sql = "SELECT * FROM activitymaster 
							INNER JOIN subjectmaster ON activitymaster.ActivitySubject = subjectmaster.SubjectCode 
							WHERE ActivityId = '$ttid'";
							...
							$asql = "SELECT * FROM studentactivity 
							INNER JOIN studentmaster ON studentactivity.SActivityUploaderId = studentmaster.StudentId 
							WHERE ActivityId = '$ttid'";
							...
							<div class="card">
								<div class="card-body">
									<h3 class="header-title">
										<?php echo $row['ActivityTitle']; ?> Info:
									</h3>
									<br>
									<div class="input-group">
										<span class="input-group-text col-3 ">Title</span>
										<input type="text" value="<?php echo $row['ActivityTitle']; ?>" aria-label="First name" class="form-control" disabled>
									...
									<div class="d-flex justify col-3 mt-3">
										<!-- Button -->
										<a href="../src/uploads/activities/<?php echo $row['ActivityFile']; ?>" download="<?php echo $row['ActivityFile']; ?>" class="btn btn-success" name="Download">
											Download
										</a>
									</div>
								</div>
										...
										<ul class="nav nav-tabs nav-overflow header-tabs">
											<li class="nav-item">
												<a href="#" class="nav-link text-nowrap active">
													Activity Submissions <span class="badge rounded-pill bg-soft-secondary"><?php echo mysqli_num_rows($aresult); ?></span>
												</a>
											</li>
										</ul>
									</div>
											...
											<table class="table table-sm table-hover table-nowrap card-table">
												<thead>
													<tr>
														...
														<th>
															<a class="list-sort text-muted" data-sort="item-score">Activity Status</a>
														</th>
														<th>
												...
												<tbody class="list font-size-base">
													...
													while ($row = mysqli_fetch_assoc($aresult)) { ?>
														<tr>
															...
															<td>
																<!-- Email -->
																<span type="text" class="text-reset item-phone" name="bsem" required><?php echo $row['SActivityUploadDate']; ?></span>
															</td>
															<td>
																<?php if ($status != 3) { // Only show action buttons if not already scored 
																?>
																<a href="act_status.php?upid=<?php echo $row['SActivityUploaderId']; ?>&asid=<?php echo $ttid; ?>&a=<?php echo 1; ?>" class="btn btn-sm btn-danger">
																	Reject
																</a>
																&nbsp;
																<button class="btn btn-sm btn-success score-btn"
																		data-assignment-id="<?php echo $ttid; ?>"
																		data-uploader-id="<?php echo $row['SActivityUploaderId']; ?>"
																...
																<?php } else { ?>
																	...
																	&nbsp;
																<?php } ?>
																<a href="../src/uploads/studentActivity/<?php echo $row['SActivityFile']; ?>" download="<?php echo $row['SActivityFile']; ?>" class="btn btn-sm btn-info" name="Download">
																	Download
																</a>
															</td>
														</tr>
			...
			<script>
				...
				form.addEventListener('submit', function(e) {
					...
					if (!currentAssignmentId || !currentUploaderId) {
						alert("Error: Activity data not found.");
						return;
					}
