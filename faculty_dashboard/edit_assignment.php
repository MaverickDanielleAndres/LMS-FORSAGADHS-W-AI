...
<?php
	if (isset($_POST['subbed'])) {
		...
		if (!empty($fs_name)) { // Only process file if a new file was uploaded
			if ($fs_error === 0) {
				if ($fs_size <= 5000000) {
		...
		if ($fileUpdateSuccess) {
			// Update SQL query - only update file if new file was uploaded
			if (!empty($fs_name)) {
				$sql = "UPDATE assignmentmaster SET 
			...
			if ($run == true) {
				echo "<script>
					document.addEventListener('DOMContentLoaded', function() {
						showCustomPopup('Assignment Updated Successfully!', 'success');
						setTimeout(function() {
				...
				echo "<script>
					document.addEventListener('DOMContentLoaded', function() {
						showCustomPopup('Error Occurred, Assignment Not Updated', 'error');
						setTimeout(function() {
							window.location.href = 'edit_assignment.php?assid=$assid';
						}, 2000);
					});
				</script>";
			}
		...
		// Removed the syntax error - this was causing the issue
		// echo "<script>window.open('assignment_list.php','_self')";  // Missing closing parenthesis and semicolon
	}
}
?>